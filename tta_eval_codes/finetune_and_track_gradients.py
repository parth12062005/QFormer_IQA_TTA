...
#####  TRAIN / EVAL LOOPS
##### ------------- ####
def train_one_epoch(qformer, regressor, dataloader, optimizer, criterion, device, epoch_gradients):
    qformer.train()
    regressor.train()
    ...
    for step, batch in enumerate(pbar, 1):
        ...
        step_grads = {}
        for name, p in qformer.model.Qformer.named_parameters():
            if p.requires_grad and p.grad is not None:
                step_grads[name] = p.grad.detach().cpu().clone()
        epoch_gradients.append(step_grads)
...
def main():
    ...
    parser.add_argument("--unfreeze_layer", type=int, default=-1,
                        help="If >= 0, unfreezes ONLY this specific layer of the Q-Former (0-11).")
    parser.add_argument("--unfreeze_strategy", type=str, default="all",
                        choices=["all", "layernorm", "query"],
                        help="What parts of the Q-Former to unfreeze if unfreeze_layer is -1.")
    ...
    # Always unfreeze regressor during supervised fine-tuning
    for p in regressor.parameters():
        p.requires_grad = True

    if args.unfreeze_layer >= 0:
        layer_prefix = f"layer.{args.unfreeze_layer}."
        print(f"  Unfreezing: Layer {args.unfreeze_layer} + Query Tokens + Regressor")
        qformer.model.query_tokens.requires_grad = True
        for name, p in qformer.model.Qformer.named_parameters():
            if layer_prefix in name:
                p.requires_grad = True
    elif args.unfreeze_strategy == "layernorm":
        print("  Unfreezing: LayerNorms + Query Tokens + Regressor")
        qformer.model.query_tokens.requires_grad = True
        for m in qformer.model.Qformer.modules():
            if isinstance(m, nn.LayerNorm):
                for p in m.parameters():
                    p.requires_grad = True
    elif args.unfreeze_strategy == "query":
        print("  Unfreezing: Query Tokens + Regressor ONLY")
        qformer.model.query_tokens.requires_grad = True
    else:
        print("  Unfreezing: Full Q-Former + Query Tokens + Regressor")
        qformer.model.query_tokens.requires_grad = True
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss = train_one_epoch(qformer, regressor, train_loader, optimizer, criterion, device, all_gradients)
    ...
    # Save the tracked gradients
    grad_file = os.path.join(args.output_dir, "layernorm_gradients.pt")
    torch.save(all_gradients, grad_file)
    print(f"[Saved] Gradient vectors tracking {len(all_gradients)} steps saved to {grad_file}")
