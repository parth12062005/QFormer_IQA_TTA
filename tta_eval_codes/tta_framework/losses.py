...
# ---------------------------------------------------------------------------
#  5) LLM-TTA Loss — Bashir et al.
# ---------------------------------------------------------------------------
class LLMTTALoss(GCLoss):
    """
    LLM-TTA Loss.
    Uses exactly the same group contrastive formulation as GCLoss,
    but clusters the batch based on external VLM pseudo-scores rather than
    ...
    def __call__(self, ctx: dict) -> torch.Tensor:
        if "vlm_scores" not in ctx or ctx["vlm_scores"] is None:
            raise ValueError("LLMTTALoss requires '--vlm_scores' JSON file to be provided!")
            
        # Temporarily override predictions with VLM scores for clustering
        orig_predictions = ctx.get("predictions")
        ctx["predictions"] = ctx["vlm_scores"]
        
        # Execute standard GCLoss math
        loss = super().__call__(ctx)
        
        # Restore
        if orig_predictions is not None:
            ctx["predictions"] = orig_predictions
            
        return loss

# ---------------------------------------------------------------------------
class LLMTTALoss(TTALoss):
    """
    LLM-TTA Loss (Bashir et al.)
    Uses external VLM pseudo-scores (or base model predictions) to cluster 
    the batch into C1 (high) and C2 (low).
    Applies FAGC-style contrastive loss (denominator only considers opposite cluster)
    using Cosine Similarity as defined in the LLM-TTA framework.
    """
    name = "llm_tta"
    requires_augmentations = False
    requires_vgg = False
    ...
    def __call__(self, ctx: dict) -> torch.Tensor:
        features = ctx["proj_feats"]   # (B, D)
        scores   = ctx["predictions"]  # (B,) -> This is vlm_scores if provided!
        device   = ctx["device"]
        ...
        # 1. Cluster into C1 and C2 based on predicted scores
        half = B // 2
        sorted_idx = torch.argsort(scores, descending=True)
        c1_idx = sorted_idx[:half]
        c2_idx = sorted_idx[half:]

        c1_feats = features[c1_idx]
        c2_feats = features[c2_idx]
        n1, n2 = c1_feats.size(0), c2_feats.size(0)
        ...
        z1 = torch.nn.functional.normalize(c1_feats, dim=1)
        z2 = torch.nn.functional.normalize(c2_feats, dim=1)

        loss = 0.0
        count = 0

        # --- Anchors in C1, positives in C1, negatives in C2 ---
        sim_c1_c1 = torch.mm(z1, z1.t()) / self.temperature
        sim_c1_c2 = torch.mm(z1, z2.t()) / self.temperature
        neg_sum_c1 = torch.sum(torch.exp(sim_c1_c2), dim=1)
        ...
        # --- Anchors in C2, positives in C2, negatives in C1 ---
        if n2 >= 2:
            sim_c2_c2 = torch.mm(z2, z2.t()) / self.temperature
            sim_c2_c1 = torch.mm(z2, z1.t()) / self.temperature
            neg_sum_c2 = torch.sum(torch.exp(sim_c2_c1), dim=1)
