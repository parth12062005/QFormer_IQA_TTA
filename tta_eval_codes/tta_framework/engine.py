...
class TTAEngine:
    ...
    def _run_tta(self, batch, image_embeds, prompts, descs):
        ...
        # Override clustering scores if VLM pseudo-scores are provided
        if "vlm_scores" in batch:
            cluster_scores = batch["vlm_scores"].to(self.device, non_blocking=True)
        else:
            cluster_scores = init_preds
        ...
        if self.warmup_proj_steps > 0:
            ...
            for step in range(warmup_steps):
                ...
                ctx = self._build_ctx(
                    proj_feats, cluster_scores, mm_embeds,
