...
def get_crossattention_query_params(qformer_module):
    """Collect only query weight/bias parameters from the cross-attention layers of the Q-Former."""
    params = []
    for name, param in qformer_module.named_parameters():
        if 'crossattention.self.query' in name:
            params.append(param)
    return params


def get_crossattention_query_026_params(qformer_module):
    """Collect only query weight/bias parameters from cross-attention layers 0, 2, and 6 of the Q-Former."""
    params = []
    target_layers = ['layer.0.', 'layer.2.', 'layer.6.']
    for name, param in qformer_module.named_parameters():
        if 'crossattention.self.query' in name:
            if any(tl in name for tl in target_layers):
                params.append(param)
    return params
...
def get_tta_params(qformer_wrapper, strategy: str):
    ...
    elif strategy == "crossattn_query":
        return get_crossattention_query_params(qformer_wrapper.model.Qformer)
    elif strategy == "crossattn_query_026":
        return get_crossattention_query_026_params(qformer_wrapper.model.Qformer)
    else:
        raise ValueError(f"Unknown unfreeze strategy: '{strategy}'. Choose from: none, layernorm, self_attn_ln, query, both, crossattn_query, crossattn_query_026")
...
def print_param_summary(qformer_wrapper, strategy: str):
    ...
    if strategy == "crossattn_query":
        caq_params = get_crossattention_query_params(qformer_wrapper.model.Qformer)
        caq_total = sum(p.numel() for p in caq_params)
        print(f"  - Cross-Attention Query Params: {caq_total} params")

    if strategy == "crossattn_query_026":
        caq_params = get_crossattention_query_026_params(qformer_wrapper.model.Qformer)
        caq_total = sum(p.numel() for p in caq_params)
        print(f"  - Cross-Attention Query Params (Layers 0,2,6): {caq_total} params")
