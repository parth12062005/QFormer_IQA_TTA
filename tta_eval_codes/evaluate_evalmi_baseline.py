"""
Evaluate baseline Q-Former checkpoint on EvalMI dataset (all splits).

Reports per-split and combined SRCC + PLCC.
Saves:
  - Per-sample predictions CSV for each split
  - Summary CSV with SRCC/PLCC per split and combined

Usage:
    python evaluate_evalmi_baseline.py [--checkpoint PATH] [--batch_size N]
...
DEFAULT_TRAIN_CSV = os.path.join(_SPLIT_DIR, "evalmi_train_full_gen_responses_PT1.csv")
DEFAULT_VAL_CSV   = os.path.join(_SPLIT_DIR, "evalmi_val_full_gen_responses_PT1.csv")
DEFAULT_TEST_CSV  = os.path.join(_SPLIT_DIR, "evalmi_test_full_gen_responses_PT1.csv")

EMBED_ROOT = "/media/parth/021f75bf-bae8-49ef-86a5-28ca19171835/parth/dataset/embeddings"
...
DEFAULT_OUTPUT_DIR = os.path.join(_SCRIPT_DIR, "results", "evalmi_baseline_splits")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate baseline Q-Former checkpoint on EvalMI (all splits)"
    )
    ...
    for split_name, csv_path in splits.items():
        ...
        per_sample_csv = os.path.join(args.output_dir, f"evalmi_{split_name}_predictions.csv")
    ...
    # --- Print summary table ---
    print("\n" + "=" * 60)
    print("  EVALUATION RESULTS — EvalMI Baseline Q-Former")
    print("=" * 60)
    ...
    combined_images_csv = os.path.join(args.output_dir, "evalmi_baseline_all_images.csv")
    ...
    summary_csv = os.path.join(args.output_dir, "evalmi_baseline_summary.csv")
