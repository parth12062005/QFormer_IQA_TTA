"""
Evaluate baseline Q-Former checkpoint on AGIQA-3K dataset (all splits).
...
Usage:
    python evaluate_a3k_baseline.py [--checkpoint PATH] [--batch_size N]

NOTE: Uses PRECOMPUTED ViT embeddings. Run precompute_embeddings_a3k.py first.
"""
...
DEFAULT_TRAIN_CSV = os.path.join(_SPLIT_DIR, "a3k_train_full_gen_responses_PT1_normalized.csv")
DEFAULT_VAL_CSV   = os.path.join(_SPLIT_DIR, "a3k_val_full_gen_responses_PT1_normalized.csv")
DEFAULT_TEST_CSV  = os.path.join(_SPLIT_DIR, "a3k_test_full_gen_responses_PT1_normalized.csv")

# A3K embeddings are flat files (e.g. AttnGAN_normal_000.npz) saved directly in a3k/
...
DEFAULT_OUTPUT_DIR = os.path.join(_SCRIPT_DIR, "results", "a3k_baseline_splits")
...
class QFormerEmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # A3K images have flat names like AttnGAN_normal_000.jpg -> AttnGAN_normal_000.npy
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate baseline Q-Former checkpoint on AGIQA-3K (all splits)"
    )
    ...
    for split_name, csv_path in splits.items():
        ...
        per_sample_csv = os.path.join(args.output_dir, f"a3k_{split_name}_predictions.csv")
    ...
    # --- Print summary table ---
    print("\n" + "=" * 60)
    print("  EVALUATION RESULTS — AGIQA-3K Baseline Q-Former")
    print("=" * 60)
    ...
    combined_images_csv = os.path.join(args.output_dir, "a3k_baseline_all_images.csv")
    ...
    summary_csv = os.path.join(args.output_dir, "a3k_baseline_summary.csv")
