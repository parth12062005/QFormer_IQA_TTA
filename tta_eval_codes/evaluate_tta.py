...
class TTADataset(Dataset):
    ...
    def __init__(self, csv_path, cfg, load_raw_images=False, split_filter=None):
        ...
        # Load VLM pseudo-scores if provided
        self.vlm_scores = {}
        if cfg.get("vlm_scores_path"):
            if os.path.exists(cfg["vlm_scores_path"]):
                with open(cfg["vlm_scores_path"], "r") as f:
                    self.vlm_scores = json.load(f)
            else:
                print(f"Warning: VLM scores file not found at {cfg['vlm_scores_path']}")
    ...
    def __getitem__(self, idx):
        ...
        # Attach VLM score if available
        if self.vlm_scores:
            if img_name in self.vlm_scores:
                out["vlm_score"] = torch.tensor(self.vlm_scores[img_name], dtype=torch.float32)
            else:
                out["vlm_score"] = torch.tensor(0.5, dtype=torch.float32) # Fallback pseudo-score
...
def collate_fn(batch):
    ...
    if "vlm_score" in batch[0]:
        out["vlm_scores"] = torch.stack([b["vlm_score"] for b in batch])
...
def main():
    ...
    parser.add_argument("--unfreeze", type=str, default="layernorm", choices=["none", "layernorm", "self_attn_ln", "query", "both", "crossattn_query", "crossattn_query_026"])
    ...
    parser.add_argument("--vlm_scores", type=str, default="", help="Path to precomputed VLM JSON scores")
    ...
    if args.vlm_scores:
        cfg["vlm_scores_path"] = args.vlm_scores
